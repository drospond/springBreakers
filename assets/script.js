$(document).ready(function() {
  ...
  function getHotelInfo(maxBudgetInput, departDate, destinationID) {
    ...
    $.ajax(settingsTripAdvHotel).then(function(response) {
      ...
      $("#Hotels_1")
        .find("img")
        .attr("src", response.data[0].photo.images.medium.url);
      ...
      $("#Hotels_1")
        .find("a")
        .attr("href", response.data[0].hac_offers.offers[0].link);
      // var pEl3 = $("<a>");
    ...
    $.ajax(settingsZomatoLocationDetails).then(function(response) {
      ...
      for (var i = 0; i < 5; i++) {
        if (response.best_rated_restaurant[i].restaurant.price_range < 5) {
          $("#Food_" + (i + 1))
            .find(".title")
            .text(response.best_rated_restaurant[i].restaurant.name);
          $("#Food_" + (i + 1))
            .find(".restaurant-url")
